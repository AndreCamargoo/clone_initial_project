...
class ProductBaseQueryBuilderRepository implements RepositoryProductInterface
{
    protected $tb, $db;
    ...
    {
        if (!$product = $this->findById($dto->id)) return null;

        return DBT::transaction(function () use ($dto, $product) {
            unset($dto->id);
            $dto = (array) $dto;
            $this->db->table($this->tb)->where('id', $product["id"])->update($dto);
            return (array) $this->findById($product["id"]);
        });
    }
